...
export default function ManagerPage() {
  ...
  return <AppShell name="スティン" role="店長" canManage>
    ...
    <section className="print-sheet">
      <header className="print-title"><h1>{printMode === "manager" ? "店舗管理用シフト表" : "スタッフ共有シフト表"}</h1><b>{printDays[0]?.shortDate}〜{printDays.at(-1)?.shortDate}</b><strong>ナミスタ {printMode === "manager" ? "管理表 v8" : ""}</strong></header>
      <table className={`print-table ${printMode === "staff" ? "staff-print" : "manager-print"}`}><thead><tr>{printMode === "manager" && <th className="wage-column">時給</th>}<th>氏名</th>{printDays.map((day) => <th key={day.isoDate}>{day.shortDate}<small>({day.weekday})</small></th>)}</tr></thead><tbody>{members.map((member) => <tr key={member.id}>{printMode === "manager" && <td className="wage-column">{yen.format(member.hourlyWage)}</td>}<td>{member.isNew ? "🔰 " : ""}{member.name}</td>{printDays.map((day) => <td key={day.isoDate}>{shiftText(member.id, day.isoDate)}</td>)}</tr>)}{printMode === "manager" && <><tr className="summary-row"><th colSpan={2}>バイト予想勤務時間</th>{printDays.map((day) => <th key={day.isoDate}>{dayHours(day.isoDate)}h</th>)}</tr><tr className="summary-row"><th colSpan={2}>予想人件費合計</th>{printDays.map((day) => <th key={day.isoDate}>{yen.format(dayLabor(day.isoDate))}</th>)}</tr></>}</tbody></table>
      {printMode === "manager" && <><div className="print-summary"><div><small>スタッフ数</small><b>{members.length}人</b></div><div><small>バイト予想勤務時間</small><b>{printDays.reduce((sum, day) => sum + dayHours(day.isoDate), 0)}時間</b></div><div><small>予想人件費合計</small><b>{yen.format(printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0))}</b></div></div><div className="management-print"><table><tbody><tr><th>人件費予算（半月目標）</th><td>{yen.format(laborBudget / 2)}</td></tr><tr><th>人件費（予想）</th><td>{yen.format(printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0))}</td></tr><tr><th>月間売上目標</th><td>{yen.format(salesTarget)}</td></tr><tr><th>半月人件費率（目標）</th><td>{laborRate}%</td></tr><tr><th>半月人件費率（予想）</th><td>{salesTarget ? (printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0) / (salesTarget / 2) * 100).toFixed(1) : "0.0"}%</td></tr></tbody></table><div className="management-notes"><h2>{printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0) <= laborBudget / 2 ? "人件費予算内" : "目標未達成"}</h2><div><b>理由・確認事項</b></div><div><b>改善策</b></div></div></div></>}
    </section>
  </AppShell>;
}
