...
export default function ManagerPage() {
  ...
  return <AppShell name="スティン" role="店長" canManage>
    ...
    <section className="print-sheet">
      ...
      {printMode === "manager" && <><div className="print-summary"><div><small>期間スタッフ数</small><b>{members.length}人</b></div><div><small>期間予想勤務時間</small><b>{printDays.reduce((sum, day) => sum + dayHours(day.isoDate), 0)}時間</b></div><div><small>期間予想人件費</small><b>{yen.format(printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0))}</b></div></div><div className="management-print"><table><tbody><tr><th>月間売上目標</th><td>{yen.format(salesTarget)}</td></tr><tr><th>目標人件費率</th><td>{laborRate}%</td></tr><tr><th>半月人件費予算</th><td>{yen.format(laborBudget / 2)}</td></tr><tr><th>半月予想人件費</th><td>{yen.format(printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0))}</td></tr><tr><th>予算との差</th><td>{yen.format(laborBudget / 2 - printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0))}</td></tr></tbody></table><div className="management-notes"><h2>{printDays.reduce((sum, day) => sum + dayLabor(day.isoDate), 0) <= laborBudget / 2 ? "人件費予算内" : "人件費予算超過"}</h2><div><b>確認事項</b></div><div><b>改善メモ</b></div></div></div></>}
    </section>
  </AppShell>;
}
