...
import { Banknote, CalendarDays, CheckCircle2, ChefHat, ChevronLeft, ChevronRight, Clock3, Plus, Printer, RefreshCw, Save, Settings2, Trash2, UsersRound, Wine } from "lucide-react";
...
export default function ManagerPage() {
  ...
  function addMember() { const index = members.length; setMembers((current) => [...current, { id: `staff-${Date.now()}`, name: "新しいスタッフ", role: "staff", jobTitle: "ホール", isNew: true, hourlyWage: 1200, color: nextStaffColor(index), skills: { kitchen: 0, hall: 1, drink: 0 } }]); setTab("skills"); setPublished(false); }
  function deleteMember(id: string) { setMembers((current) => current.filter((member) => member.id !== id)); setAssignments((current) => current.filter((item) => item.staffId !== id)); setPublished(false); }
    ...
    {tab === "shift" && <>
      ...
      <div className="grid gap-5 xl:grid-cols-[240px_1fr]">
        ...
        <section className="min-w-0 space-y-5">
          ...
          {!!assignments.length && <div className="flex flex-col gap-3 rounded-2xl border bg-white p-3 sm:flex-row sm:justify-end"><div className="flex gap-2 sm:mr-auto"><select value={printHalf} onChange={(e) => setPrintHalf(e.target.value as "first" | "second")} className="min-w-0 flex-1 rounded-xl border px-3 sm:flex-none"><option value="first">前半（1〜16日）</option><option value="second">後半（17日〜末日）</option></select><button onClick={printSchedule} className="flex items-center justify-center gap-2 rounded-xl border px-4 py-3 font-bold"><Printer size={17}/>A4で印刷・PDF保存</button></div><button onClick={publish} className="flex items-center justify-center gap-2 rounded-xl bg-[#29262d] px-6 py-3 font-bold text-white"><Save size={17}/>確定してスタッフに公開</button>{published && <span className="self-center text-sm font-bold text-emerald-700">公開済み</span>}</div>}
        </section>
      </div>
    </>}
